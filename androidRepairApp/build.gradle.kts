...
android {
    namespace = "io.github.dai2010.focuseed.repair"
    compileSdk = 35

    defaultConfig {
        applicationId = "io.github.dai2010.focuseed.repair"
        minSdk = 29
        targetSdk = 35
        versionCode = 1
        versionName = "0.1.3-Android-DND-repair1"
    }
...
tasks.matching { it.name == "assembleRelease" || it.name == "packageRelease" }.configureEach {
    doFirst {
        check(hasReleaseSigning) {
            "Android repair release signing secrets are required."
        }
    }
}
